...
@Getter
public class RequirementRadiationPerTick implements IRequirement<RadiationComponent, Void> {
  public static final NamedCodec<RequirementRadiationPerTick> CODEC = NamedCodec.record(instance -> instance.group(
      IOType.CODEC.fieldOf("mode").forGetter(RequirementRadiationPerTick::getMode),
      NamedCodec.doubleRange(0D, Double.MAX_VALUE).fieldOf("amount").forGetter(RequirementRadiationPerTick::getAmount),
      NamedCodec.intRange(0, Integer.MAX_VALUE).optionalFieldOf("range", () -> 80 /*5 chunks (default Mekanism config value)*/).forGetter(RequirementRadiationPerTick::getRadius)
  ).apply(instance, RequirementRadiationPerTick::new), "Requirement Radiation Per Tick");
  ...
  @Override
  public RequirementType<RequirementRadiationPerTick, RadiationComponent, Void> getType() {
    return RequirementTypeRegistration.RADIATION_PER_TICK.get();
  }
  ...
  private CraftingResult processTick(RadiationComponent component, ICraftingContext context) {
    return getMode().isInput() ? processInput(component, context) : processOutput(component, context);
  }
  ...
  public void getDefaultDisplayInfo(IDisplayInfo info, RecipeRequirement<?, ?, ?> requirement) {
    if(getMode().isInput()) {
      ...
      info.addTooltip(Component.translatable("modular_machinery_reborn.jei.ingredient.radiation.pertick.input", sievert(this.amount), this.radius));
    }
    else {
      info.setItemIcon(MekanismItems.GEIGER_COUNTER.asItem());
      info.addTooltip(Component.translatable("modular_machinery_reborn.jei.ingredient.radiation.pertick.output", sievert(this.amount)));
    }
  }
