...
public class RequirementHeatPerTick implements IRequirement<HeatComponent, BasicHeatCapacitor> {
  public static final NamedCodec<RequirementHeatPerTick> CODEC = NamedCodec.record(instance -> instance.group(
  ...
  ).apply(instance, RequirementHeatPerTick::new), "RequirementHeatPerTick");
  ...
  public RequirementHeatPerTick(double amount, IOType mode, PositionedRequirement position) {
    if (RecipeModifier.blacklist.stream().noneMatch(RequirementTypeRegistration.HEAT_PER_TICK::equals))
      RecipeModifier.addToBlacklist(RequirementTypeRegistration.HEAT_PER_TICK);
    this.amount = amount;
    this.mode = mode;
  ...
  @Override
  public RequirementType<RequirementHeatPerTick, HeatComponent, BasicHeatCapacitor> getType() {
    return RequirementTypeRegistration.HEAT_PER_TICK.get();
  }
  ...
  public boolean test(HeatComponent component, ICraftingContext context) {
    double amount = context.getPerTickModifiedValue((float) this.amount, this);
    if (getMode().isInput()) {
